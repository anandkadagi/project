<%-- 
    ...
    Created on : Dec 6, 2023, 5:44:53 PM
    Author     : ASUS
--%>
...
<link rel="stylesheet" type="text/css" href="https://common.olemiss.edu/_js/sweet-alert/sweet-alert.css">
                 ...
                 <%
                    ...
                    SimpleDateFormat inputDateFormat = new SimpleDateFormat("yyyy-MM");
                     ...
                     SimpleDateFormat outputMonthFormat = new SimpleDateFormat("MM");
                      String month1 = outputMonthFormat.format(date);
   SimpleDateFormat outputMonthFormat1 = new SimpleDateFormat("YYYY");
                ...
                String status2="cancelled";
                  PreparedStatement pst=Connection1.con.prepareStatement("select *  from orders where status='"+status1+"' OR status='"+status2+"' INTERSECT select *  from orders where (MONTH(date)='"+month1+"' AND YEAR(date)='"+year+"')",ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
        ResultSet rs=pst.executeQuery();
                    pst=Connection1.con.prepareStatement("select count(order_id) as orders,sum(if(status='"+status1+"',amount,0)) as total ,sum(status='"+status1+"') as stat1 ,sum(status='"+status2+"') as stat2 from orders where (status='"+status1+"' OR status='"+status2+"') AND (MONTH(date)='"+month1+"' AND YEAR(date)='"+year+"')"  ,ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
...
{
%>
                <img src="Images/document.png" height="100px" width="100px" style="align-items: center"/>
